  ...
  <div class="main">
    <div>
      <div class="components-list bg-indigo-lighten-5 pa-4 rounded-lg">
        <div class="components-header text-h6 mb-4 text-indigo-darken-2">
          Components
        </div>
    ...
    <div class="page">
      ...
      >
        ...
        >
          ...
          />
          <g
            v-for="(connection, index) in loopVisualConnections"
            :key="`loop-${index}`"
          >
            <path
              :d="connection.path"
...
<script setup lang="ts">
    ...
    const startItem = items.find(
      (item) =>
        !items.some(
          (other) =>
            (other.type === 'Decision' &&
              (findNearestRight(other, items) === item ||
                findNearestBottom(other, items) === item)) ||
  ...
  const updateCanvasSize = () => {
    if (canvasRef.value) {
      ...
      const maxItemX = Math.max(
        ...canvasItems.value.map(
          (item) =>
            item.x + (item.type === 'Loop' ? LOOP_WIDTH : COMPONENT_WIDTH)
        )
      );
      const maxItemY = Math.max(
        ...canvasItems.value.map(
          (item) =>
            item.y + (item.type === 'Loop' ? LOOP_HEIGHT : COMPONENT_HEIGHT)
        )
      );
      ...
      const newHeight = Math.max(
        rect.height,
        maxItemY + 100 * Y_AXIS_EXPANSION_FACTOR,
        window.innerHeight
      );
  ...
  const autoScroll = () => {
    if (canvasRef.value) {
      ...
      const heightDiff =
        canvasSize.value.height - previousCanvasSize.value.height;
  ...
  const createLoopVisualConnection = (
    from: CanvasItem,
    to: CanvasItem,
    color: string,
    style: string = 'dashed'
  ...
  const loopVisualConnections = computed((): Connection[] => {
    ...
    items.forEach((item) => {
      if (item.type === 'LoopStart') {
        ...
        if (loopEnd) {
          lines.push(
            createLoopVisualConnection(item, loopEnd, '#FFA500', 'dashed')
          );
        }
      }
    });
  ...
  const runCompile = async (
    sortedCanvasItems: SortedCanvasItem | null
  ): Promise<void> => {
    code = ''; // 코드 초기화
