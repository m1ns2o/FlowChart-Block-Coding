...
>
  <v-icon size="24" color="red darken-1">mdi-delete</v-icon>  <!-- 아이콘 색상을 빨간색으로 변경 -->
</v-btn>
    ...
    <v-dialog v-model="classDeleteDialog" max-width="300px">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="blue darken-1" @click="closeClassDeleteDialog">취소</v-btn>
...
const deleteClass = async () => {
  try {
    ...
    console.log(response.data)
    localStorage.clear();
    router.push('/class') // 클래스 목록 페이지로 이동
  } catch (error) {
